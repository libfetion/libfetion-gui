...
char *encryPWD(const char *data, long datalen, bool isEncry)
{
	...
	if (isEncry)
	{
		return encryptXOR(data, datalen , KEY);
	}
	else
		return decryptXOR(data, datalen , KEY);
#else
	char *tmp = NULL;
	char *res = NULL;
	if (isEncry)
	{
		if (tmp = fillcode(data))
		{
			qt_debug("encry tmp [%s] \n", tmp);
			res = DES_3(tmp, strlen(tmp), isEncry);
			qt_debug("encry res [%s] \n", res);
		}
	} else {
		tmp = DES_3(data, datalen, isEncry);
		res = resolvecode(tmp);
	}
