    ...
    : QDialog(parent)
{
	...
	connect(RB_Accept, SIGNAL(clicked()), this, SLOT(verifyRB()));
	connect(RB_Refuse, SIGNAL(clicked()), this, SLOT(verifyRB()));

	connect(buttonBox, SIGNAL(accepted()), this, SLOT(verifyAccount()));
...
void FxVerifyAccount::verifyAccount()
{
	if (RB_Accept->isChecked())
	{
		...
		fx_handleContactRequest(m_uri, 1, groupId, Local_name->text().toUtf8().data());
	} else {
