...
namespace fxgui
{
    ...
    FxWidget::FxWidget(QWidget *parent,
                       Qt::WindowFlags flag): QWidget(parent, flag)
    {
        ...
        connect(this, SIGNAL(triggleMaximizeAndNormal()),
                titleBar->btnMaximize, SLOT(click()));
    ...
    void FxWidget::setBackground(QPixmap pix)
    {
        ...
        painter.drawImage(left, 0, image.copy(left, 0, image.width() - left -
                          right, image.height()).scaled(width() - left - right,
                          image.height()), 0, 0, width() - left - right, top);
        painter.drawImage(width() - right, 0, image, image.width() - right, 0,
                          right, top);

        //middle
        QImage image2 = image.copy(0, top, image.width(), image.height() - top
                                   - bottom).scaled(image.width(), height() -
                                   top - bottom);
        painter.drawImage(0, top, image2, 0, 0, left, height() - top - bottom);
        painter.drawImage(left, top, image2.copy(left, 0, image.width() - left
                          - right, image2.height()).scaled(width() - left -
                          right, height() - top - bottom), 0, 0, width() - left
                          - right, height() - top - bottom);
        painter.drawImage(width() - right, top, image2, image2.width() - right,
                          0, right, height() - bottom - top);
        ...
        painter.drawImage(left, bottomDesY, image.copy(left, 0, image.width() -
                          right - left, image.height()).scaled(width() - left -
                          right, image.height()), 0, bottomSrcY, width() - left
                          - right, bottom);
    ...
    void FxWidget::endAutoHide()
    {
        ...
        switch (positionState)
        {
            ...
            case WP_RIGHT:
                ...
                resize(orientSize);
                move(QApplication::desktop()->width() - orientSize.width() - 1,
                     y());
                turnBackNormal();
                break;
            case WP_TOP:
            ...
            case WP_BOTTOM:
                resize(orientSize);
                move(x(),
                     QApplication::desktop()->height() - orientSize.height());
                turnBackNormal();
                break;
            default:
                break;
