...
bool init_db()
{
	...
	if( sqlite3_get_table(pdb, sql, &result, &nrow, &ncol, &perrmsg) != SQLITE_OK)
	{
		//create the table
		memset(sql, 0, SQL_MAXLEN);
		sprintf(sql, "create table %s(flag, id, len_id, pwd, len_pwd, login_state)", AUTOLG);
...
bool isAutoLogin(char** id, char** pwd, int* loginState)
{
	...
	memset(sql, 0, SQL_MAXLEN);
	sprintf(sql,"select id, pwd, len_id, len_pwd, login_state from %s where flag=\"1\" ", AUTOLG);
...
bool setAutoLogin(const char* id, const char*pwd, int login_state)
{
	...
	memset(sql, 0, SQL_MAXLEN);
	sprintf(sql,"insert into %s values(\"1\", \"%s\", \"%d\", \"%s\", \"%d\", \"%d\")",
			AUTOLG, en_id, strlen(id), en_pwd, strlen(pwd), login_state);
	sqlite3_exec(pdb, sql, 0, 0, 0);
...
bool setProxy(PROXY_ITEM *proxy_item)
{
	...
	sprintf(sql,"insert into %s values(\"%d\", \"%s\", \"%s\", \"%s\", \"%s\")",
			PROXY, proxy_item->type, proxy_item->host, proxy_item->port, 
...
BOOL init_histroy_db(long usr)
{
	...
	if (sqlite3_get_table(pdb, sql, &result, &nrow, &ncol, &perrmsg) != SQLITE_OK)
	{
		...
		//create the table
		memset(sql, 0, SQL_MAXLEN);
		sprintf(sql, "create table fx%ldusr(uid,msg text, msg_len, date_time NOT NULL DEFAULT CURRENT_TIMESTAMP)", usr);
...
int  export_history(long uid, const char* filename)
{
	...
	if(SQLITE_OK != sqlite3_get_table(pdb, sql, &result, &nrow, &ncol, &perrmsg)){
	...
	QTextStream out(&htm);
	out << "<html><head><meta http-equiv=Content-Type content=\"text/html;charset=gb2312\"><title>LibFx chatlog</title></head><body>"
		<< res << "</body></html>";
...
char *create_sql_for_histroy(long usr, long uid, HISTORY_DATE type)
{
	...
	switch(type)
	{
		...
		case ALL_DATE_HISTORY:
			sprintf(sql,"select * from fx%ldusr where uid == \"%ld\" order by date_time asc", usr, uid);
			break;
		default:
			return NULL;
	}
...
bool selectSystemMsg(long usr, long uid, const char* msg)
{
	...
	memset(sql, 0, SQL_MAXLEN);
	sprintf(sql, "select * from fx%ldusr where msg_len == \"%ld\" and uid == \"%ld\" ", 
			usr, len_msg, uid);
...
BOOL init_storeAccountInfo_db(long usr)
{
	...
	//create the table
	memset(sql, 0, SQL_MAXLEN);
	sprintf(sql, "create table fxACINFO%ldusr(uid, online_state, mobilenm, local_name, nickname, impresa, showname)", usr);
	//exec the create table sql
	if( sqlite3_exec(pdb, sql, 0, 0, &perrmsg) != SQLITE_OK )
	{
		printf("exel create table %s\n", sql);
...
void create_search_result(	QList<QTreeWidgetItem *> *items, char **result, int nrow, int ncol);
QList<QTreeWidgetItem *> *searchAccountInfo(char * keyword)
{
	...
	sprintf(sql, "select * from fxACINFO%ldusr where uid like \'%s%%\' or mobilenm like \'%s%%\' or local_name like \'%%%s%%\' or  nickname like \'%%%s%%\' or impresa like \'%%%s%%\'",
			usr, keyword, keyword, keyword, keyword, keyword);
	...
	*/
	...
	for(int i = 0; i < nrow; i++)
	{
		...
		if (fx_is_pc_user_by_id(ac_info->accountID)){
			...
			info += "<b style=\"color:red; \">"+QString("%1").arg(ac_info->accountID)+"</b>";
			tips += info +"<br>";
		}
