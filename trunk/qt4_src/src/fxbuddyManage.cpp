...
BuddyMge::BuddyMge(QTreeWidget * widget, FxMainWindow *wind)
{
	...
	connect(treeWidget, SIGNAL(itemDoubleClicked (QTreeWidgetItem* , int) ), 
			this, SLOT(slot_accountDoubleClicked ( QTreeWidgetItem *, int)));
	connect(treeWidget, SIGNAL(itemPressed (QTreeWidgetItem* , int) ), 
			this, SLOT(slot_accountPressed ( QTreeWidgetItem *, int)));
}
...
void BuddyMge::initAllActions()
{
	...
	connect(RefreshInfoBuddyAct, SIGNAL(triggered()), this, SLOT(slot_updateInfoBuddy()));
	...
	connect(AddBlackBuddyAct, SIGNAL(triggered()), this, SLOT(slot_addBlackBuddy()));

	RemoveBlackBuddyAct = new QAction(tr("remove frome black list"), getMainWindow());
	RemoveBlackBuddyAct->setIcon(getMenuIcon(RemoveBlackIcon));
	connect(RemoveBlackBuddyAct, SIGNAL(triggered()), this, SLOT(slot_removeBlackBuddy()));
...
void BuddyMge::createGroupMenu(QMenu *groupMenu)
{
	...
	while(tmp_group)
	{
		...
		if(group) {
			QAction *action = new QAction(QString::fromUtf8(group->name), getMainWindow());
...
{
	bool ok;
	QString text = QInputDialog::getText(getMainWindow(), tr("addGroup"),
...
void BuddyMge::slot_deleteGroup()
{
	...
	{
        QMessageBox::warning(getMainWindow(), tr("can not delete group"), tr("group is not null"),QMessageBox::Yes);
		return;	
	}
...
void BuddyMge::slot_renameGroup()
{
	...
	bool ok;
	QString text = QInputDialog::getText(getMainWindow(), tr("renameGroup"),
...
void BuddyMge::slot_renameBuddy()
{
	...
	QString text = QInputDialog::getText(getMainWindow(), tr("renamebudd"),
