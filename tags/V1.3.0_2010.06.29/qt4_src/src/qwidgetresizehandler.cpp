...
#include "qwidgetresizehandler_p.h"
    ...
    namespace fxgui
    {
        ...
        {
            if (!isActive() || (ee->type() != QEvent::MouseButtonPress && ee
                ->type() != QEvent::MouseButtonRelease && ee->type() != QEvent
                ::MouseMove && ee->type() != QEvent::KeyPress && ee->type() !=
            ...
            switch (e->type())
            {
                case QEvent::MouseButtonPress:
                {
                    ...
                    if (!widget->rect().contains(widget->mapFromGlobal(e
                        ->globalPos())))
                    {
                        return false;
                    }
                    if (e->button() == Qt::LeftButton)
                    {
                        ...
                        invertedMoveOffset = widget->rect().bottomRight() -
                            moveOffset;
                        if (mode == Center)
                        {
                            if (movingEnabled)
                ...
                break;
                case QEvent::MouseButtonRelease: if (w->isMaximized())
                {
                    break;
                }
                if (e->button() == Qt::LeftButton)
                ...
                case QEvent::MouseMove:
                {
                    ...
                    buttonDown = buttonDown && (e->buttons() &Qt::LeftButton);
                        // safety, state machine broken!
                    bool me = movingEnabled;
                ...
                break;
                case QEvent::KeyPress: keyPressEvent((QKeyEvent*)e);
                break;
                case QEvent::ShortcutOverride: if (buttonDown)
                {
                    ((QKeyEvent*)ee)->accept();
                    return true;
                }
        ...
        void QWidgetResizeHandler::mouseMoveEvent(QMouseEvent *e)
        {
            ...
            if (!moveResizeMode && !buttonDown)
            {
                ...
                else if (pos.y() >= widget->height() - range && pos.x() >=
                         widget->width() - range)
                {
                    mode = BottomRight;
                }
            ...
            QPoint globalPos = (!widget->isWindow() && widget->parentWidget())
                                ? widget->parentWidget()->mapFromGlobal(e
                                ->globalPos()): e->globalPos();
            if (!widget->isWindow() && !widget->parentWidget()->rect().contains
                (globalPos))
            {
                if (globalPos.x() < 0)
                {
                ...
                if (sizeprotect && globalPos.x() > widget->parentWidget()
                    ->width())
                {
                    globalPos.rx() = widget->parentWidget()->width();
                }
                if (sizeprotect && globalPos.y() > widget->parentWidget()
                    ->height())
                {
            ...
            QSize mpsize(widget->geometry().right() - pp.x() + 1, widget
                         ->geometry().bottom() - pp.y() + 1);
            mpsize = mpsize.expandedTo(widget->minimumSize()).expandedTo(QSize
                                       (mw, mh)).boundedTo(maxsize);
            QPoint mp(widget->geometry().right() - mpsize.width() + 1, widget
            ...
            switch (mode)
            {
                ...
                case BottomRight:
                    ...
                    break;
                case BottomLeft:
                    geom = QRect(QPoint(mp.x(), widget->geometry().y()), QPoint
                                 (widget->geometry().right(), p.y()));
                    break;
                case TopRight:
                    geom = QRect(QPoint(widget->geometry().x(), mp.y()), QPoint
                                 (p.x(), widget->geometry().bottom()));
                    break;
                case Top:
                ...
                case Left:
                    ...
                    break;
                case Right:
                    geom = QRect(widget->geometry().topLeft(), QPoint(p.x(),
            ...
            geom = QRect(geom.topLeft(), geom.size().expandedTo(widget
                         ->minimumSize()).expandedTo(QSize(mw, mh)).boundedTo
                         (maxsize));

            if (geom != widget->geometry() && (widget->isWindow() || widget
                ->parentWidget()->rect().intersects(geom)))
            {
                if (mode == Center)
                {
        ...
        void QWidgetResizeHandler::setMouseCursor(MousePosition m)
        {
                ...
                for (int i = 0; i < children.size(); ++i)
                {
                    ...
                    {
                        if (!w->testAttribute(Qt::WA_SetCursor) && !w->inherits
                            ("QWorkspaceTitleBar"))
                        {
        ...
        void QWidgetResizeHandler::keyPressEvent(QKeyEvent *e)
        {
            ...
            switch (e->key())
            {
                ...
                break;
                case Qt::Key_Right: pos.rx() += delta;
                ...
                case Qt::Key_Space: case Qt::Key_Return: case Qt::Key_Enter:
                    case Qt::Key_Escape: moveResizeMode = false;
                widget->releaseMouse();
                widget->releaseKeyboard();
                buttonDown = false;
                break;
